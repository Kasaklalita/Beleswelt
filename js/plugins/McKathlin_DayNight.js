...
/*:
 * @target MZ
 * @plugindesc v2.0 Configure & track a day-night cycle. Optional blood moon.
 * @author McKathlin
 * 
 ...
 *    creating.
 * 4. Adding "McKathlin" to your game's credits is greatly appreciated, but
 *    not required.
...
(() => {
	...
	PluginManager.registerCommand(pluginName, "resetLighting", args => {
		...
		if(duration) {
			$gameMap.lightingType = McKathlin.DayNight.getLightingNotetag($dataMap.note);
			$gameMap.mapTone = McKathlin.DayNightCycle.getToneByKeyword($gameMap.lightingType);
			$gameScreen.startTint($gameMap.mapTone, duration);
		}
	});
	...
	PluginManager.registerCommand(pluginName, "useLightingPreset", args => {
		var duration = Number(args.duration);
		if(args.lightingKeyword && duration) {
			$gameMap.lightingType = args.lightingKeyword;
			$gameMap.mapTone = McKathlin.DayNightCycle.getToneByKeyword($gameMap.lightingType);
			$gameScreen.startTint($gameMap.mapTone, duration);
		}
	});
	...
	if (McKathlin.DayNight.Param.EnableBloodmoon) {
		...
		McKathlin.DayNight.updateTime_noBloodmoon =
			McKathlin.DayNightCycle.updateTime;
		McKathlin.DayNightCycle.updateTime = function() {
			McKathlin.DayNight.updateTime_noBloodmoon.call(this);
		...
		// Alias method
		McKathlin.DayNight.getOutsideTone_noBloodmoon = 
			McKathlin.DayNightCycle.getOutsideTone;
		McKathlin.DayNightCycle.getOutsideTone = function() {
			if (!this.isBloodmoonPhase(true)) {
				return McKathlin.DayNight.getOutsideTone_noBloodmoon.call(this);
			}
			var time = this.getMinutesOfDay();
			if (time < McKathlin.DayNight.Param.DawnStartTimeAsMinutes) {
				// night, between midnight and dawn
